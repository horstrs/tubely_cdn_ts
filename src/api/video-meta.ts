...
import { dbToSignedVideo } from "./videos"
...
export async function handlerVideoGet(cfg: ApiConfig, req: BunRequest) {
  ...
  const signedVideo = dbToSignedVideo(cfg, video)
...
export async function handlerVideosRetrieve(cfg: ApiConfig, req: Request) {
  ...
  for (const dbVideo of videos){
    const signedVideo = dbToSignedVideo(cfg, dbVideo);
    signedVideos.push(signedVideo);
  }
